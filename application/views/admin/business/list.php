			...
			<div class="col-xs-12 message_box">
				...
				<div class="box">
					<div class="box-header with-border">  
                		...
                		<form method="get" action="<?php echo base_url().'admin/business/list';?>"> 
						<div class="box-body row"> 
							<div class="form-group col-md-3">
                                <input class="column_filter form-control" id="business_name" name='business_name' type="text" placeholder="Business Name" value="<?php if(!empty($filter_business_name))echo $filter_business_name;?>">
			              	</div>
                        	<div class="form-group col-md-3">
							...
							<a class="btn btn-default" href="<?php echo base_url().'admin/business/list';?>">Reset</a>
							</div>
			          	</div>
			          </form> 
								...
								{	
									...
									foreach ($records_results as $row) { $i++; 
										...
										<tr id="tr_<?php echo $row[$field]; ?>">
											...
											<td><?php if(!empty($row['category'])) echo $row['category'];?></td>
											<td><?php if(!empty($row['state'])) echo $row['state'];?></td>
											<td><?php if(!empty($row['city'])) echo $row['city'];?></td>
											<td><?php if(!empty($row['bookings'])) echo $row['bookings'];?></td>
											...
											<?php }?>
											<td class="td-actions">
												<!-- <a id="" href="<?php echo $view_action.'/'.$row[$field]; ?>" class="btn btn-xs btn-primary" title="" data-toggle="tooltip" data-original-title="Edit">
														<i class="fa fa-eye"></i>
													</a> -->
												<?php /*if(isset($edit_action) && !empty($edit_action)){ ?>
