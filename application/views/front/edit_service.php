    ...
    <div class="wrapper h-100vh">
    	...
    	<div class="bottomSection mh-auto  mt-3">
        	<div class="container mini-container formSection">
            	<div class="row">
            		<form method="POST" id="b_service_frm" action="<?php echo base_url("update_service"); ?>" role="form" data-parsley-validate>
            			<input type="hidden" name="service_id" value="<?php echo $detail['id'];?>">
                	<div class="col-12">
	                    <div class="mb-3">
	                        <input type="text" class="form-control" placeholder="<?php echo $this->lang->line('lab_service_name_text');?>" name="service_name" value="<?php echo $detail['service_name'];?>" maxlength="50" data-parsley-required data-parsley-required-message="<?php echo $this->lang->line('err_service_name_text');?>" data-parsley-errors-container="#err_service_name"/>
	                    </div>
	                    <span id="err_service_name"></span>
	                    <div class="mb-3">
	                        <input type="text" class="form-control" placeholder="<?php echo $this->lang->line('lab_service_person_text');?>" value="<?php echo $detail['service_person'];?>" name="service_person" maxlength="50" data-parsley-required data-parsley-required-message="<?php echo $this->lang->line('err_service_person_text');?>" data-parsley-errors-container="#err_service_person"/>
	                    </div>
	                    <span id="err_service_person"></span>
	                    <div class="mb-3">
	                        <input type="text" class="form-control" placeholder="<?php echo $this->lang->line('lab_service_time_text');?>" name="service_time" value="<?php echo $detail['service_time'];?>" oninput="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" maxlength="3" data-parsley-required data-parsley-required-message="<?php echo $this->lang->line('err_service_time_text');?>" data-parsley-errors-container="#err_service_time"/>
	                    </div>
	                    <span id="err_service_time"></span>
	                    <div class="mb-3">
	                        <select name="gender" id="" class="form-control">
	                            <option value="male" <?php echo ($detail['gender']=='male')?'selected':'';?>>Male</option>
	                            <option value="female" <?php echo ($detail['gender']=='female')?'selected':'';?>>Female</option>
	                        </select>
	                    </div>
    	...
    	<div class="container">
        	<div class="row">
            	<div class="col-12">
                    ...
                    <button class="btn shadow bg-theme-outline w-100 d-flex justify-content-between" onclick="deleteBtn()" id="b_service_frm_delt_btn">
                     <span class="mx-auto"><?php echo $this->lang->line('btn_delete_service');?></span>   
                    </button>
            	</div>
        	</div>
    	</div>
...
function deleteBtn() {
	...
	var id="b_service_frm";
	var form = $('#'+id)[0];
	if(confirm(`<?php echo $this->lang->line('info_are_you_sure');?>`)){
		var formData = new FormData(form);
		$("#loader").show(); 
		$("#"+id+"_btn").attr('disabled',true);
		$("#b_service_frm_delt_btn").attr('disabled',true);
		$.ajax({
			...
			success:function(resp){
				$("#loader").hide();  
				$("#"+id+"_btn").attr('disabled',false);
				$("#b_service_frm_delt_btn").attr('disabled',false);
				if(resp.status==1){
					swal(resp.msg);
					setTimeout(function() {
						location.href=resp.returnurl;
